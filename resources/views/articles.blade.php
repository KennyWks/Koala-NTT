...
@if($rows->count())
    <div class="card mb-3">
        ...
        <p>
            <small class="text-muted">By <a class="text-decoration-none" href="/articles?author={{$rows[0]->author->username}}">{{$rows[0]->author->name}}</a> in <a href="/articles?category={{$rows[0]->category->slug}}" class="text-decoration-none">{{$rows[0]->category->name}}</a>  {{$rows[0]->created_at->diffForHumans()}}
            </small>
        </p>
...
<div class="container">
    <div class="row">
        ...
        <div class="col-md-4 mb-3">
            <div class="card">
                <div class="position-absolute px-3 py-3 text-white" style="background-color:rgba(0, 0, 0, 0.7)"><a class="text-decoration-none" href="/articles?category={{$row->category->slug}}">{{$row->category->name}}</a></div>
